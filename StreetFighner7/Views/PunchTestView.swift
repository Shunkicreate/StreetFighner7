...
struct PunchTestView: View {
    ...
    var body: some View {
        HStack {
            ...
            VStack(spacing: 20) {
                Button(action: {
                    motionManager.startGyro(interval: 0.1)
                }, label: {
                    Text("ジャイロセンサー開始")
                })
                Button(action: {
                ...
                Text("ジャイロ")
                Text("x: \(motionManager.gyroData?.rotationRate.x ?? 0)")
                Text("y: \(motionManager.gyroData?.rotationRate.y ?? 0)")
                Text("z: \(motionManager.gyroData?.rotationRate.z ?? 0)")
